...
def summ():
    ...
    answer = mb.askretrycancel(title='',message='Сложить еще 3 числа?')
    if answer:
        e1.delete(0,END)
        e2.delete(0,END)
        e3.delete(0,END)
...
def multiply():
    ...
    u_2 = int(s2)
    u_3 = int(s3)
    multiply = u_1* u_2 * u_3
    m1["text"] = f'{s1}*{s2}*{s3}={multiply}'
    
    answer = mb.askretrycancel(title='',message='Умножить еще 3 числа?')
    if answer:
        e1.delete(0,END)
        e2.delete(0,END)
        e3.delete(0,END)
...
m = Label(height=3, text='Введите 3 целых числа и нажмите на кнопку.')
m.pack()
e1 = Entry()
e1.pack()
e2 = Entry()
